...
export default function OnboardingForm() {
  ...
  const [data, setData] = useState({
    state: '', lga: '', crop: '', farmSize: '', language: '',
  })
            ...
            {[0, 1, 2].map((s) => (
              <div
                ...
                className={`h-1.5 rounded-full transition-all duration-500 ${
                  s === step ? 'w-10 bg-forest' : s < step ? 'w-6 bg-forest/50' : 'w-6 bg-earth/20'
                }`}
              />
            ))}
          </div>
          <div className="text-xs text-earth/70 font-mono">{step + 1} / 3</div>
        </div>
